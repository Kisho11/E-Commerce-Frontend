...
const PREVIEW_FIRST_PAGE_UNITS = 9;
const PREVIEW_CONTINUATION_PAGE_UNITS = 21;
const PREVIEW_SUMMARY_UNITS = 6;
...
function getPreviewItemUnits(item) {
  const variant = formatVariant(item);
  const textLength = `${item?.name || ''} ${variant}`.trim().length;

  return Math.max(1, Math.ceil(textLength / 90));
}

function buildInvoicePreviewPages(items = []) {
  ...
  let currentPage = {
    ...
    showSummary: false,
    usedUnits: 0,
    capacity: PREVIEW_FIRST_PAGE_UNITS,
  };
  ...
  const startNextPage = () => {
    ...
    currentPage = {
      ...
      showSummary: false,
      usedUnits: 0,
      capacity: PREVIEW_CONTINUATION_PAGE_UNITS,
    };
  };

  sourceItems.forEach((item) => {
    const units = getPreviewItemUnits(item);
  ...
  if (currentPage.usedUnits + PREVIEW_SUMMARY_UNITS <= currentPage.capacity) {
    ...
    pages.push({
      ...
      showIntro: false,
      showSummary: true,
      usedUnits: PREVIEW_SUMMARY_UNITS,
      capacity: PREVIEW_CONTINUATION_PAGE_UNITS,
    });
  }
...
function OrderDetailsModal({ order, onClose, accentClass = 'text-primary' }) {
  ...
  const previewPages = buildInvoicePreviewPages(order.items);
        ...
        <div className="order-print-content overflow-y-scroll bg-gray-100 px-3 py-5 text-sm sm:px-6">
          {previewPages.map((page, pageIndex) => (
            <article
              ...
              className="invoice-a4-page relative mx-auto mb-6 flex min-h-[297mm] w-[210mm] max-w-full flex-col bg-white px-[12mm] pb-[30mm] pt-[12mm] text-slate-900 shadow-sm"
            >
              {page.showIntro && (
                <>
              ...
              {page.items.length > 0 || (page.showIntro && !order.items?.length) ? (
                <section className={page.showIntro ? 'mt-10' : 'mt-0'}>
                  <table className="w-full table-fixed border-collapse text-sm">
                    <thead className="bg-slate-900 text-white">
                      <tr>
                        <th className="w-12 px-2 py-2 text-left font-bold">S.No</th>
                        <th className="px-2 py-2 text-left font-bold">Description</th>
                        <th className="w-14 px-2 py-2 text-right font-bold">Qty</th>
                        <th className="w-20 px-2 py-2 text-right font-bold">Rate</th>
                    ...
                    <tbody>
                      {page.items.length > 0 ? (
                        page.items.map((item) => {
                          ...
                          return (
                            <tr key={item.lineId || `${item.id}-${item.name}-${originalIndex}`} className="break-inside-avoid border-b border-gray-200">
                              <td className="px-2 py-3 align-top">{originalIndex + 1}</td>
                              <td className="px-2 py-3 align-top">
                                <p className="font-semibold leading-snug text-slate-900">{item.name || '-'}</p>
                                {variant && <p className="mt-1 text-xs leading-snug text-slate-500">{variant}</p>}
                              </td>
                              <td className="px-2 py-3 text-right align-top">{item.quantity || 0}</td>
                              <td className="px-2 py-3 text-right align-top">{formatCurrency(item.price)}</td>
                              <td className="px-2 py-3 text-right align-top font-semibold">{formatCurrency(lineTotal)}</td>
                            </tr>
                          );
                        })
                      ) : (
              ...
              {page.showSummary && (
                <section className="mt-auto grid break-inside-avoid gap-8 pt-10 md:grid-cols-[1fr_260px]">
              ...
              <footer className="absolute bottom-[8mm] left-[12mm] right-[12mm] border-t border-gray-300 pt-[4mm] text-center text-xs font-bold text-slate-800">
                {BANK_DETAILS}
              </footer>
            </article>
          ))}
