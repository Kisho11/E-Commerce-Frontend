...
function OrderDetailsModal({ order, onClose, accentClass = 'text-primary' }) {
  ...
  const previewPages = buildInvoicePages(order.items);
              ...
              {page.items.length > 0 || (page.showIntro && !order.items?.length) ? (
                <section className={page.showIntro ? 'mt-10' : 'mt-0'}>
                  <table className="w-full table-fixed border-collapse text-sm">
                    ...
                    <tbody>
                      {page.items.length > 0 ? (
                        page.items.map(({ item, itemNumber }) => {
                          const variant = formatVariant(item);
                          const lineTotal = (Number(item.price) || 0) * (Number(item.quantity) || 0);

                          return (
                            <tr key={item.lineId || `${item.id}-${item.name}-${itemNumber}`} className="break-inside-avoid border-b border-gray-200">
                              <td className="px-2 py-3 align-top">{itemNumber}</td>
